...
must_haves:
  truths:
    - "An order can be created linking a buyer, provider, and gig with pricing snapshot"
    - "An order has one of five visible statuses: Pending, Accepted, In Progress, Completed, or Cancelled"
    - "Only valid status changes are allowed (e.g., a completed order cannot go back to in progress)"
    - "Placing an order requires selecting a valid pricing tier"
  artifacts:
